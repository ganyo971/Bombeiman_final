...
{
	assert(map);
	return (x>=0 && x<map->width && y>=0 && y<map->height); // pour limiter le déplacement du joueur à la map
...
void map_free(struct map* map)
{
	if (map == NULL )
		return;
	free(map->grid);   // ptet que c'est une fction à faire
	free(map);         // idem
}
...
{
	switch (type >> 4) {         // pour faire apparaître les monstres
	case CELL_MONSTER:
...
void map_display(struct map* map)
{
	...
	for (int i = 0; i < map->width; i++) {
		for (int j = 0; j < map->height; j++) {
			...
			unsigned char type = map->grid[CELL(i,j)];    // je pense que c'est ici qu'il faut modifier pour faire apparaître les monstres
			...
			case CELL_KEY:
				...
				break;
			case CELL_MONSTER:                    // Ici le monstre à l'image d'une key (qui ressemble à une bombe je ne sais pas pk)
				window_display_image(sprite_get_key(), x, y);
				break;
			case CELL_DOOR:
...
struct map* map_get_default(void)
{
	...
	// moification effectuer dans ce code: remplacement d'un empty par une porte!!! à modifier

	unsigned char themap[MAP_WIDTH * MAP_HEIGHT] = {
			CELL_PLAYER, CELL_EMPTY, CELL_EMPTY, CELL_EMPTY, CELL_EMPTY, CELL_DOOR, CELL_EMPTY, CELL_EMPTY, CELL_EMPTY, CELL_EMPTY, CELL_EMPTY, CELL_EMPTY,
