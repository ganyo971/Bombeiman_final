...
// Sprites of Monsters                                       //truc rajouté pour monstre
...
// player
SDL_Surface* player_img[4];        // elle sert à rien cette ligne??? à définir player_img!! ptet qu'en définissant un seul
							 // pas besoin de def 1,2 et 3. ps: qd on met 1 a la place de 4 la touche bas ne fctionne plus correctemt

// monster
SDL_Surface* monster_img[4];      // truc rajouté pour monstre
...
void monster_load() {										// truc rajouté pour affichage monstre
...
void sprite_load() {
	...
	banner_load();
	player_load();
	monster_load();        // truc rajouté pour monstre
}
...
void sprite_free() {
	...
	banner_unload();
	player_unload();
	monster_unload();            // truc rajouté pour monster
}
...
SDL_Surface* sprite_get_player(enum direction direction) {
	...
	return player_img[direction];
                                                               ...
                                                               // image monstre dépendant de la direction
